...
MAX_ACCEL_PROFILES = {
    AccelPersonality.eco:       [2.0,  1.90,  1.60, 1.15, .500, .292, .13, .09],  # eco
    AccelPersonality.normal:    [2.0,  1.95,  1.85, 1.30, .650, .310, .17, .12],   # normal
...
MIN_ACCEL_PROFILES = {
    LongPersonality.relaxed:    [-.0020, -.0020, -.2, -0.4, -1.1],  # gentler braking
    LongPersonality.standard:   [-.0010, -.0010, -.3, -0.5, -1.2],  # normal braking
    LongPersonality.aggressive: [-.0008, -.0008, -.4, -0.6, -1.2],  # more aggressive braking
}
MIN_ACCEL_BREAKPOINTS =         [0.,   1.5,    3.,   15.,  50.]

# Follow distance profiles mapped to LongPersonality (relaxed/standard/aggressive)
FOLLOW_PROFILES = {
    LongPersonality.relaxed:    [1.75, 1.80, 1.75, 1.65, 1.70, 1.80, 1.80],  # more spread out
    LongPersonality.standard:   [1.35, 1.40, 1.35, 1.35, 1.55, 1.50, 1.50],  # balanced
    LongPersonality.aggressive: [1.05, 1.10, 1.05, 1.05, 1.30, 1.25, 1.27],  # tighter
}
