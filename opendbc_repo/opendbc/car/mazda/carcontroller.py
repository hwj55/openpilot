...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      # 修改處：加入大腦正向加速度指令，讓實驗模式綠燈可自動起步
      restart_requested = starting or CC.actuators.accel > 0.0
      ...
      if CC.longActive:
        ...
        if release_brake:
          accel = max(accel, 0.0)
        # 修改處：加入 not release_hold_requested，當駕駛踩油門時不再鎖死煞車
        elif CS.out.standstill and not release_hold_requested:
