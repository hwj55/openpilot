...
from tools.tuning.lat_feedforward import get_steer_feedforward_han2022 
...
def fit(speed, angle, steer, angle_plot=True):
  ...
  BOUNDS = ([0.001, 0.01, 0.4, 0., 0.1, 0.1, 0.001, 0., 0.],
          ...
          ([0.001, 0.02, -3., 0.0, 0.1, 0.1, 0.1, 0.0, 0.0],
          [50., 1.23, 3., 40., 0.5, 0.5, 2.0, 1.0, 140.0])
  ...
  os.mkdir("plots")
  os.mkdir("plots/kph_img")
  os.mkdir("plots/steer_img")
...
def plot(speed, angle, steer):
  f_data_distribution = open("plots/data_distribution.txt","a")
  if SPEED_PLOTS:
    # os.system('rm plots/steer_img')
    ...
    for angle_range in _angles:
      if IS_ANGLE_PLOT:
        ...
        angle_range_str = f'deg_{start:02d}-{end:02d}'
      else:
        start = angle_range[0]
        end = angle_range[1]
        angle_range_str = f'lat_accel_{start:.2f}-{end:.2f}'
      ...
      plt.scatter(plot_speed,
                  plot_steer,
                  label=angle_range_str,
                  color='black',
                  s=1.)

      _x_ff = np.linspace(0, SPEED_MAX, res)
      _y_ff = [ old_feedforward(_i, np.mean(angle_range)) for _i in _x_ff ]
      plt.plot(_x_ff, _y_ff,  color='red', label='old')

      _y_ff = [ new_feedforward(_i, np.mean(angle_range)) for _i in _x_ff ]
      ...
      if not os.path.isdir('plots'):
        os.mkdir('plots')
      plt.savefig(f'plots/steer_img/{angle_range_str}.png')
      plt.close()
  
    print(''.join([ "{}: {}{}".format(k,v,'\n' if (ki+1) % 1 == 0 else ', ') for ki,(k,v) in enumerate(sorted(list(angle_points_dict.items()), key=lambda x: x[0]))]))
    f_data_distribution.write(''.join([ "{}: {}{}".format(k,v,'\n' if (ki+1) % 1 == 0 else ', ') for ki,(k,v) in enumerate(sorted(list(angle_points_dict.items()), key=lambda x: x[0]))]))
  ...
  if ANGLE_PLOTS:
    ...
    for speed_range in _speeds:
      start_kph = speed_range[0]*CV.MS_TO_KPH
      end_kph = speed_range[1]*CV.MS_TO_KPH
      speed_range_str = f'kph_{start_kph:.0f}-{end_kph:.0f}'
      mask = (speed_range[0] <= speed) & (speed < speed_range[1])
      ...
      if FIT_EACH_PLOT and sum(mask) > 4:
        try:
           # lgtm[py/mismatched-multiple-assignment] pylint: disable=unbalanced-tuple-unpacking
          params, _ = curve_fit(  _fit_kf,
            np.array([plot_speed, plot_angle]),
            np.array(plot_steer),
            maxfev=9000,
          )
      ...
      _y_ff = [ old_feedforward(np.mean(speed_range), _i) for _i in _x_ff]
      plt.plot(_x_ff, _y_ff, color='red', label='old')

      _y_ff = [ get_steer_feedforward_han2022(np.mean(speed_range), _i) for _i in _x_ff ]
      plt.plot(_x_ff, _y_ff, color='blue', label='new')
      ...
      plt.savefig(f'plots/kph_img/{speed_range_str}.png')
      plt.close()

    print(''.join([ "{}:{}{}".format(k,v,'\n' if (ki+1) % 1 == 0 else ', ') for ki,(k,v) in enumerate(list(angle_points_dict.items()))]))
    f_data_distribution.write(''.join([ "{}:{}{}".format(k,v,'\n' if (ki+1) % 1 == 0 else ', ') for ki,(k,v) in enumerate(list(angle_points_dict.items()))]))

    f_data_distribution.close()

  if SPEED_PLOTS or ANGLE_PLOTS:
    # Create animations
    cmds = [
      f'rm -rf ~/Downloads/plots',
      # 'convert -delay 40 plots/steer_img.png deg-up.gif',
      ...
      'convert -delay 40 plots/kph_img/*.png kph-up.gif',
      'convert -reverse kph-up.gif kph-down.gif',
      'convert -loop -1 kph-up.gif kph-down.gif kph.gif',
      # 'convert -loop -1 deg.gif kph.gif solution.gif',
      'mv *.gif plots/',
      'mv plots ~/Downloads/',
      'rm -f ~/Downloads/plots/steer_img/*.png',
    #   'rm -f ~/Downloads/plots/lat*.png',
      # 'rm -f ~/Downloads/plots/kph_img.png',
      # 'rm -f regularized',
