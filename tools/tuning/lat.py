...
def process_file(rlog_dir_list, latsegs, outpath, path):
    for rlog_path in rlog_dir_list:
      ...
      if filename not in latsegs:
          try:
            print(f"Loading segment file: {rlog_path}")
      ...
      if PREPROCESS_ONLY:
        if "realdata" in path or (preprocess and dongleid):
          ...
          if preprocess and dongleid:
            dongle_id = dongleid
            rlog_path = path #"/Users/haiiro/Downloads/latfiles_batch"
            rlog_log_path = outpath #"/Users/haiiro/Downloads/latfiles.txt" # prevents rerunning rlogs
          else:
          ...
          for filename in tqdm(filenames, desc="Preparing fit data from rlogs"):
            if len(filename.split('--')) == 3 and f"{dongle_id}|{filename}.lat" not in latsegs:
              with tempfile.TemporaryDirectory() as d:
                ...
                elif os.path.exists(os.path.join(path,filename,"rlog")):
                  tmpbz2 = os.path.join(d,f"{dongle_id}_{filename}--rlog")
                  shutil.copy(os.path.join(path,filename,"rlog"),tmpbz2)
        ...
        #   处理过的数据就不重复处理了
          ...
          for filename in os.listdir(outpath):
            if ext in filename.split("/")[-1]:
              p1 = filename.replace(outpath, path).replace('.lat','--rlog').replace('|','_')
              p2 = filename.replace(outpath, path).replace('.lat','--rlog')
          ...
          for folder in os.listdir(path):
            for filename in os.listdir(os.path.join(path,folder)):
              if filename.endswith("rlog") and filename not in latsegs:
                rlog_dir_list.append(os.path.join(path,folder,filename))


        #   filenames = sorted([filename for filename in os.listdir(path) if filename.endswith("rlog") and filename not in latsegs])
          
          process_file(rlog_dir_list, latsegs, outpath, path)
        #   p_map(process_file, rlog_dir_list, desc="Preparing fit data from rlogs")
          if preprocess:
            for filename in filenames:
                latsegs.add(filename.replace(outpath, path).replace('.lat','--rlog').replace('|','_'))
                latsegs.add(filename.replace(outpath, path).replace('.lat','--rlog'))
            with open(rlog_log_path, 'w') as rll:
              for ls in sorted(list(latsegs)):
                rll.write(f"\n{ls}")
          
          # for filename in tqdm(filenames, desc="Preparing fit data from rlogs"):
          #   if len(filename.split('--')) == 4 and filename.endswith('rlog'):
