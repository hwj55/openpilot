...
def gen_long_model():
  ...
  x_ego_dot, v_ego_dot, a_ego_dot = SX.sym('x_ego_dot'), SX.sym('v_ego_dot'), SX.sym('a_ego_dot')
  model.xdot = vertcat(x_ego_dot, v_ego_dot, a_ego_dot)
  a_min, a_max, x_obstacle, a_prev, lead_t_follow, lead_danger_factor = [SX.sym(s) for s in ['a_min', 'a_max', 'x_obstacle', 'a_prev', 'lead_t_follow', 'lead_danger_factor']]
...
def gen_long_ocp():
  ...
  j_ego = ocp.model.u[0]
  a_min, a_max, x_obstacle, a_prev, lead_t_follow, lead_danger_factor = [ocp.model.p[i] for i in range(6)]
  ...
  ocp.cost.zl = ocp.cost.Zl = ocp.cost.Zu = ocp.cost.zu = cost_weights
...
class LongitudinalMpc:
  ...
  def reset(self):
    ...
    self.solve_time = self.time_qp_solution = self.time_linearization = self.time_integrator = 0.0
  ...
  def process_lead(self, lead):
    ...
    if lead is not None and lead.status:
      ...
      x_lead, v_lead, a_lead, a_lead_tau = 50.0, v_ego + 10.0, 0.0, _LEAD_ACCEL_TAU
  ...
  # 修改：新增 a_cruise_min_override 支援
