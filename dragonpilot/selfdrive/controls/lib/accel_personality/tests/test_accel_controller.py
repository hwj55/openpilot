...
from dragonpilot.selfdrive.controls.lib.accel_personality.accel_controller import (
...
class TestAccelPersonalityController:
  ...
  def controller(self, mock_params, mocker):
    mocker.patch('dragonpilot.selfdrive.controls.lib.accel_personality.accel_controller.Params', return_value=mock_params)
  ...
  def test_load_personality_invalid_value(self, mocker):
    # 測試給定錯誤的數值 (不在選項內)，應該要安全退回 normal
    mock_params = mocker.Mock()
  ...
  def test_load_personality_parse_error(self, mocker):
    # 測試給定無法轉換為整數的字串，應該要捕獲 ValueError 並退回 normal
    mock_params = mocker.Mock()
  ...
  def test_get_accel_personality(self, controller):
    controller._accel_personality = AccelPersonality.sport
  ...
  def test_cycle_accel_personality_full_cycle(self, controller):
    controller._accel_personality = AccelPersonality.eco
  ...
  def test_get_accel_limits_at_zero_speed(self, controller, personality):
    controller._accel_personality = personality
  ...
  def test_is_enabled_true(self, controller):
    controller._enabled = True
    assert controller.is_enabled() is True

  def test_is_enabled_false(self, controller):
  ...
  def test_is_enabled_calls_params(self, controller, mock_params):
    mock_params.get_bool.assert_any_call('AccelPersonalityEnabled')
    call_count_before = mock_params.get_bool.call_count
    for _ in range(20):
      controller.update()
    assert mock_params.get_bool.call_count > call_count_before
  ...
  def test_max_accel_decreases_with_speed(self, controller):
    ...
    for personality in [AccelPersonality.eco, AccelPersonality.normal, AccelPersonality.sport]:
      ...
      for v in test_speeds:
        max_accels.append(controller.get_max_accel(v))
        # 模擬系統時間推進，確保快取機制允許重新計算
        controller.frame += 1
  ...
  def test_acceleration_personality_ordering(self, controller, v_ego):
    ...
    controller._accel_personality = AccelPersonality.normal
    controller.first_run = True
