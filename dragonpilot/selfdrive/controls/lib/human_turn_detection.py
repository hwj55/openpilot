...
class HumanTurnDetection:
  def __init__(self) -> None:
    ...
    # 【效能優化】只在初始化時讀取一次參數，徹底避免 100Hz 迴圈中的 I/O 延遲
    ...
    self._angle_release_deg = self._get_float("dp_htd_turn_angle_release_deg", 20.0)
    self._torque_start_nm = self._get_float("dp_htd_torque_start_nm", 2.0)
    self._torque_release_nm = self._get_float("dp_htd_torque_release_nm", 0.6)
    self._dtsc_torque_nm = self._get_float("dp_htd_dtsc_torque_nm", 1.0)
    self._resume_angle_lock_deg = self._get_float("dp_htd_resume_angle_lock_deg", 90.0)
  ...
  def update(self, lat_active: bool, cruise_enabled: bool, steering_angle_deg: float, steering_torque_nm: float,
             v_ego: float, steering_pressed: bool = False) -> tuple[bool, HTDState]:
    
    # 【修改】移除 self._read_params()，讓迴圈保持極致流暢，不再發生卡頓
    ...
    # [新增] 定速巡航鎖：當定速巡航啟用時，強制讓 HTD 暫停作動 (保持 INACTIVE)
    if cruise_enabled:
      if self._state != HTDState.INACTIVE:
        self._transition(HTDState.INACTIVE, "cruise_active")
      self._trigger_start_time = 0.0
      # 直接回傳 True (代表允許 OP 繼續接管橫向控制)，並維持 INACTIVE 狀態
    ...
    if time.monotonic() - self._state_change_time >= self._dynamic_delay:
      ...
      # [新增安全鎖 5] 接管角度鎖：若方向盤角度仍大於安全閥值 (預設 90 度)，拒絕恢復自動駕駛
  ...
  def _should_trigger(self) -> bool:
    ...
    condition_met = (self._last_pressed and 
                     direction_match and
  ...
  @property
  def htd_action_active(self) -> bool:
    """
    判斷是否為人工干預：
    1. 施加扭力 >= DTSC 專屬門檻 (預設 1.0 nm)
    2. 轉向與扭力方向一致
    3. steering_pressed == True (真實握盤，無幽靈扭力)
    """
    direction_match = (self._last_angle_raw * self._last_torque_raw) > 0
    # [修改] 使用獨立的 DTSC 扭力門檻，取代原本寫死的 2.0
    return self._last_pressed and direction_match and self._last_torque >= self._dtsc_torque_nm
